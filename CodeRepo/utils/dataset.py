...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        X = self.data[idx]
        target = self.gt[idx]
        new_labes_id = [20, 21, 27, 30, 36, 38, 42, 45, 46, 48, 49, 50, 51]
        label_mapping = {k: v for v, k in enumerate(new_labes_id)}
        # Convert numpy array to torch tensor
        X = torch.from_numpy(X)
        # target = torch.from_numpy(np.array(target)).float()


        target = torch.from_numpy(np.array(label_mapping[target])).float()
        # if self.eval_mode:
        #     X = X.view()
        #     target = target.view()
...
colordict = {'B04': '#a6cee3', 'NDWI': '#1f78b4', 'NDVI': '#b2df8a', 'RATIOVVVH': '#33a02c', 'B09': '#fb9a99',
             'B8A': '#e31a1c', 'IRECI': '#fdbf6f', 'B07': '#ff7f00', 'B12': '#cab2d6', 'B02': '#6a3d9a',
             'B03': '#0f1b5f',
...
if __name__ == "__main__":
    data_path =  "D:\II_LAB3_DATA\imgint_trainset_v2.hdf5"
